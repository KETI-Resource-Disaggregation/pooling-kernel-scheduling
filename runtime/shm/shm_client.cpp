...
//
// 흐름:
//   kraken_shm_open(group_id):
//     1. shm_open("/kraken_{group_id}") — controller가 미리 생성해 둔 shm
//     2. ftruncate는 controller 담당 (runtime은 읽기/쓰기만)
//     3. mmap → g_kraken.shm
//     4. magic 확인
//     5. 빈 슬롯 탐색 → g_kraken.tenant_idx 등록
//     6. tenants[idx].active = 1
//
//   kraken_shm_close():
//     1. tenants[idx].active = 0 (cuda_hooks kraken_fini에서 이미 처리)
//     2. munmap
...
#define ENV_VIRTUAL_SM     "KRAKEN_VIRTUAL_SM"
#define ENV_VIRTUAL_MEM_MB "KRAKEN_VIRTUAL_MEM_MB"
...
void kraken_shm_open(const char* group_id) {
    ...
    if (fd < 0) {
        fprintf(stderr, "[kraken] shm_open 실패: %s\n", path);
        return;
    }

    void* addr = mmap(nullptr, sizeof(KrakenSharedState),
                      PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);

    if (addr == MAP_FAILED) {
        fprintf(stderr, "[kraken] mmap 실패\n");
        return;
    }

    KrakenSharedState* shm = (KrakenSharedState*)addr;

    // magic 확인
    if (shm->magic != SHM_MAGIC) {
        fprintf(stderr, "[kraken] shm magic 불일치 (0x%08X)\n", shm->magic);
        munmap(addr, sizeof(KrakenSharedState));
        return;
    }
    ...
    for (int i = 0; i < MAX_TENANTS; i++) {
        if (strcmp(shm->alloc[i].tenant_id, g_kraken.tenant_id) == 0) {
            idx = i;
            break;
        }
    }
    ...
    if (idx < 0) {
        fprintf(stderr, "[kraken] 슬롯 부족 (MAX_TENANTS=%d)\n", MAX_TENANTS);
        munmap(addr, sizeof(KrakenSharedState));
        return;
    }

    g_kraken.shm        = shm;
    g_kraken.tenant_idx = idx;

    // tenant_id 기록 (controller가 미리 기록했을 수도 있음)
    strncpy(shm->alloc[idx].tenant_id, g_kraken.tenant_id,
    ...
    if (shm->alloc[idx].virtual_sm == 0) {
        ...
        if (env_sm && env_mem) {
            ...
            if (vsm > 0 && vmem > 0) {
                ...
                fprintf(stderr,
                    "[kraken] env에서 할당: sm=%d mem=%dMB mps=%d%%\n",
    ...
    fprintf(stderr, "[kraken] shm 연결: %s slot=%d sm=%d mem=%dMB\n",
        path, idx,
...
void kraken_shm_close(void) {
    if (!g_kraken.shm) return;
    munmap(g_kraken.shm, sizeof(KrakenSharedState));
    g_kraken.shm = nullptr;
}
