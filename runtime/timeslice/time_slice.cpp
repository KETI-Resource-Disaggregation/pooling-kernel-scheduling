...
void timeslice_deduct(int64_t elapsed_us) {
    ...
    TenantState* me = &shm->tenants[g_kraken.tenant_idx];
...
void timeslice_add_wait(int64_t wait_us) {
    ...
    __atomic_fetch_add(&shm->tenants[g_kraken.tenant_idx].total_wait_us,
...
bool timeslice_exhausted(void) {
    ...
    int64_t remain = __atomic_load_n(
        &shm->tenants[g_kraken.tenant_idx].time_remain_us,
        __ATOMIC_RELAXED);
    return remain <= 0;
}
